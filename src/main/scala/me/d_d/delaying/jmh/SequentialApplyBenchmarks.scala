...
class SequentialApplyBenchmarks {
  ...
  @Benchmark
  def ArraySequentialApply(bh: Blackhole): Unit = {
    var i = 0
    while(i < size) {
      bh.consume(arr.apply(i))
      i += 1
    }
  }

  @Benchmark
  def DelayingArraySequentialApply(bh: Blackhole): Unit = {
    var i = 0
    while(i < size) {
      bh.consume(darr.apply(i))
      i += 1
    }
  }

  @Benchmark
  def IntRRBVectorSequentialApply(bh: Blackhole): Unit = {
    var i = 0
    while(i < size) {
      bh.consume(rrb_vec.apply(i))
      i += 1
    }
  }

  @Benchmark
  def VectorSequentialApply(bh: Blackhole): Unit = {
    var i = 0
    while(i < size) {
      bh.consume(vec.apply(i))
      i += 1
    }
  }
}

object SequentialApplyBenchmarks {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val defaults = new OptionsBuilder()
      .include(classOf[SequentialApplyBenchmarks].getSimpleName)
      .mode(Mode.AverageTime)
      .resultFormat(ResultFormatType.CSV)
      .result("apply_sequential_big.csv")
      .warmupIterations(10)
